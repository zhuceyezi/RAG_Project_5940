...
def set_dnd_theme():
    st.markdown("""
    <style>
        /* Main background and styles */
        .stApp {
            background-image: url('https://i.imgur.com/87FcJSI.jpg');
            background-size: cover;
            background-attachment: fixed;
            color: #3b2e1e;
        }
        
        /* Make all containers slightly transparent with parchment-like look */
        div[data-testid="stVerticalBlock"] > div {
            background-color: rgba(252, 246, 231, 0.85);
            border-radius: 8px;
            padding: 10px;
            margin: 10px 0;
        ...
        section[data-testid="stSidebar"] {
            background-color: rgba(58, 43, 24, 0.95);
...
if "scene_graph_img" not in st.session_state and "script_text" in st.session_state:
    with st.spinner("🧙‍♂️ The wizards are deciphering your adventure..."):
...
if st.session_state.get("show_map") and "scene_graph_img" in st.session_state:
    st.markdown('<div class="dnd-divider"></div>', unsafe_allow_html=True)
    st.header("🗺️ Adventure Map")
    st.markdown(f'<img src="data:image/png;base64,{st.session_state["scene_graph_img"]}" class="scene-map" style="width: 100%; max-width: 800px; margin: 0 auto; display: block;">', unsafe_allow_html=True)